...
class MultiRenderer:
    def __init__(
        ...
        carb.log_info(
            ...
            f"q={jpeg_quality}, settle={settle_frames}, parallel=True)"
        )

    def start(self):
        self._running = True
        self._loop_task = asyncio.ensure_future(self._render_loop())
        carb.log_info("[MultiRenderer] Parallel render loop started")
    ...
    async def _render_loop(self):
        while self._running:
            try:
                sessions = [
                    s for s in self._session_mgr.get_all_sessions()
                    if s.ws and not s.ws.closed and s.viewport_api
                ]
                ...
                if self._settle_frames > 0:
                    settle_tasks = [
                        s.viewport_api.wait_for_rendered_frames(self._settle_frames)
                        for s in sessions
                    ]
                    await asyncio.gather(*settle_tasks, return_exceptions=True)

                capture_tasks = [
                    self._capture_frame(s.viewport_api) for s in sessions
                ]
                ...
                for session, result in zip(sessions, results):
                    ...
                    frame_data, cap_w, cap_h = result

                    if not self._first_capture_logged:
                        self._first_capture_logged = True
                        carb.log_info(
                            f"[MultiRenderer] First capture: {cap_w}x{cap_h}, "
                            f"buffer={len(frame_data)} bytes, "
                            f"session={session.session_id}, "
                            f"parallel_batch={len(sessions)} users"
                        )

                    send_tasks.append(
                        self._encode_and_send(loop, session, frame_data, cap_w, cap_h)
                    )
    ...
    async def _encode_and_send(
        self, loop, session: UserSession,
        frame_data: bytes, width: int, height: int,
    ):
        try:
            frame_bytes = await loop.run_in_executor(
                self._thread_pool, self._encode_frame, frame_data, width, height
            )
            if frame_bytes and not session.ws.closed:
